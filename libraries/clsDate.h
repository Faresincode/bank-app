...
public:
	...
	clsDate(string sDate,string DayDelimiter= "/")
	{
		vector <string> vDate = clsString::Split(sDate, DayDelimiter);
		_Day = stoi(vDate[0]);
		_Month = stoi(vDate[1]);
		_Year = stoi(vDate[2]);
		_Seconds = stoi(vDate[3]);
		_Minutes = stoi(vDate[4]);
		_Hours = stoi(vDate[5]);
	}

	clsDate(short Day, short Month, short Year,short Seconds = 1, short Minutes = 1, short Hours = 1)
	{
		_Day = Day;
		_Month = Month;
		_Year = Year;
	...
	static clsDate GetSystemDateAndTime()
	{
		time_t t = time(0);
		tm* now = localtime(&t);
		short Day = 0, Month = 0, Year = 0,Seconds=0,Minutes=0,Hours=0;
		Day = now->tm_mday;
		Month = now->tm_mon + 1;
		Year = now->tm_year + 1900;
		Seconds = now->tm_sec;
		Minutes = now->tm_min;
		Hours = now->tm_hour;
		return clsDate(Day, Month, Year,Seconds,Minutes,Hours);
