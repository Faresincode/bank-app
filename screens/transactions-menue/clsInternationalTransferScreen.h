...
private:
	...
	static void _InterTransDetailsTicket(clsBankClient::stInterTransferClientInfo SenderClient,clsBankClient::stInterTransferClientInfo BeneficiaryClient, double Amount)
	{
		...
		cout << "\n" << setw(ScreenWidth) << "" << left << "bank fee  Amount       : " << BankFeesAmount<<" "<<SenderClient.BankClientInfo.CurrencyCode ;
		cout << "\n" << setw(ScreenWidth) << "" << left << "Tax Amount             : " << TaxAmount<< " " << SenderClient.BankClientInfo.CurrencyCode;
		cout << "\n" << setw(ScreenWidth) << "" << left << "Total Withdraw Amount  : " << Amount + TaxAmount + BankFeesAmount<< " " << SenderClient.BankClientInfo.CurrencyCode;
		cout << "\n" << setw(ScreenWidth) << "" << left << "New Balance            : " << SenderClient.BankClientInfo.AccountBalance<< " " << SenderClient.BankClientInfo.CurrencyCode;
